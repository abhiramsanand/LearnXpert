...
const HolidayCalendar: React.FC = () => {
  ...
  const fetchCourseDates = async () => {
    try {
      const response = await axios.get('/api/v1/courses/dates/dayNumber');
  ...
  const handleMarkHoliday = async () => {
    if (selectedDate) {
      try {
        await axios.post('/api/v1/courses/mark-holiday/day', {
  ...
  const handleUnmarkHoliday = async () => {
    if (selectedDate) {
      try {
        await axios.post('/api/v1/courses/unmark-holiday', {
          holidayDate: selectedDate.toISOString().split('T')[0],
        });
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns} locale={locales['en-US']}>
      ...
      <Dialog open={isDialogOpen} onClose={() => setDialogOpen(false)}>
        ...
        <DialogContent>
          <Button onClick={handleMarkHoliday}>Mark as Holiday</Button>
          <Button onClick={handleUnmarkHoliday}>Unmark as Holiday</Button>
        </DialogContent>
        <DialogActions>
